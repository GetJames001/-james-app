...
class ExecutiveDeliberationEngine {
  ...
  findBlockingIssues(results) {
    ...
    for (const r of normalized) {
      for (const risk of r.risks) {
        if (this.blockingRiskPatterns.some(rx => rx.test(String(risk)))) {
          blockers.push({
            advisor_id: r.advisor_id,
            risk
          });
      ...
      for (const gap of r.missing_information) {
        if (/critical|unknown|unverified|pending|unresolved/i.test(String(gap))) {
          blockers.push({
            advisor_id: r.advisor_id,
            risk: `Missing information: ${gap}`
          });
        }
      }
    }
  ...
  readiness(results, round = 1) {
    ...
    else if (unresolvedRecommendationConflict || unresolvedCriticalUncertainty) status = "deliberate";

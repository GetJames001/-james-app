...
class LiveCouncilRuntime {
  ...
  async executeSeat(assignment, context) {
    if (!assignment?.advisor_id) throw new Error("assignment.advisor_id is required");
    const adapter = this.adapters[assignment.advisor_id];
    if (!adapter) throw new Error(`No adapter for ${assignment.advisor_id}`);
    return adapter.execute(assignment, context);
  }
  ...
  targetsForNextRound(state, current) {
    ...
    return [...ids].filter(id => this.requiredSeats().includes(id));
  }
